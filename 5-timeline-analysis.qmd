...
This workflow corresponds to the full script at `scripts/2_analysis/5 timeline analysis.R`. 

The folded code block below cleans the environment, loads the data, and prepares to plot it. Feel free to click the "Code" button to see the code otherwise, we'll go straight to calculating the number of species per cat arrival category above:
...
This is the number of extinct populations per cat arrival category:
...
#| output: true
#| code-fold: true
#| fig-cap: "**Fig. 1.** Number of attributed population extinctions by taxonomic class and overlap with cats."
...
#| fig-height: 8
#| code-fold: true
#| fig-cap: "**Fig. 2.** Timeline chart of estimated mammal extinctions relative to estimated cat arrival (vertical line) per species."

plot_dt[min_difference == max_difference,
        `:=` (min_difference = min_difference - 1,
              max_difference = max_difference + 1)]

sub_plot <- plot_dt[Class == "Mammals", ]
sub_plot[, x_id := x_id - min(x_id)+1]

ggplot() +
  geom_vline(xintercept = 0)+
  # stripes
  geom_rect(
    data = unique(sub_plot[, .(
      xmin = x_id - 0.5,
      xmax = x_id + 0.5,
      stripe = x_id %% 2)]),
  ...
  labs(
    x = "Years between cat arrival and last record",
    y = NULL) +
  ggtitle("Mammals")+
  theme_lundy +
  theme(
...
#| fig-height: 9
#| code-fold: true
#| fig-cap: "**Fig. 3.** Timeline chart of estimated bird extinctions relative to estimated cat arrival (vertical line) per species."

sub_plot <- plot_dt[Class == "Birds", ]
sub_plot[, x_id := x_id - min(x_id)+1]
...
#| fig-height: 6
#| code-fold: true
#| fig-cap: "**Fig. 4.** Timeline chart of estimated reptile extinctions relative to estimated cat arrival (central line) per species."

sub_plot <- plot_dt[Class == "Reptiles", ]
sub_plot[, x_id := x_id - min(x_id)+1]
...
#| output: true
#| code-fold: true
#| fig-cap: "**Fig. 5.** Map of extinction locations colored by relationship of extinction estimate to cat arrival estimate."
...
cat_ex_map %>%
 mutate(`Prey extirpation year range` = paste(Prey_extirpation_year_earliest, 
                                               "-", Prey_extirpation_year_latest),
         `Cat arrival year range` = paste(Cat_arrival_year_earliest, 
                                               "-", Cat_arrival_year_latest)) %>%
  select(Class, Order, `Scientific name`, 
         Extinction_extirpation, `Redlist category`, cat_arrival,
         `Prey extirpation year range`,
         `Cat arrival year range`, 
         Prey_quote) %>%
  rename("Extinction or extirpation" = "Extinction_extirpation",
          "Extinction relative to arrival" = "cat_arrival",
         "Prey quote" = Prey_quote) %>%
  mapview(zcol = "Extinction relative to arrival",
        col.regions = rev(plot_col),
        layer.name = "Extinction relative to arrival",
        popup = popupTable(., zcol = c("Class",
                                     "Order",
                                     "Scientific name",
                                     "Redlist category",
                                     "Extinction relative to arrival",
