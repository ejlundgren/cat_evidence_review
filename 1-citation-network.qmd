...
The data for this analysis consists of two files, one for the nodes (e.g., each article) and one for the edges (the citations between articles). This data was prepared in `scripts/1_data_mgmt/2 create network formats.R` and were further pre-processed in `scripts/2_analysis/1 citation networks and bias.R` to be ready to be used to create our citation network.
...
# The edges/citations between articles:
edges <- fread("builds/citation_network/edges_tidied.csv")
head(edges[, .(scientificName, cited_by_id, article_id)])

# And the node information:
nodes <- fread("builds/citation_network/nodes_tidied.csv")
head(nodes[, .(node_id, article_node_name, evidence_type_fine)])
...
## Network plot
Now, let's plot it. Feel free to unfold the code to see how:
...
#| fig-height: 6
#| label: fig-network
#| fig-cap: "Citation network. Claimant sources are core sources that claim that causes are drivers of endangerment. Non-claimants are sources (including the Web of Science) that do not make this claim. Point color indicates type of article found along each citation chain."
...
See [here](1.2-citation-network-app.qmd) to explore the citation network in more detail with an interactive app.

# Probability of being cited
Now, let's evaluate the terminus of each citation chain from the core claimant sources (the large bright pink points). We used graph algorithms to identify the terminus of each citation chain, starting with the core sources (the large pink circles in the network figure above). See `scripts/1_data_mgmt/3 create probability of being cited dataset.R` for this code.

We then asked what proportion of those citation chains ended in *population studies*, e.g., empirical studies that link cats to bird abundance or reproduction ?

To do so we'll load a separate data frame produced by this that is organized as source - citation-terminus pairs. Each row is a *potential* citation. The colum `cited` (0 or 1) indicates whether the article was cited or not. Pairs are constrained by species and publication timing (e.g., articles published after ≤6 months after claimant reviews concluded were excluded).

This dataset allows us to evaluate whether claimants cited the available evidence, and whether there was any bias in citation towards articles that found a negative association between cats and prey. We'll do this with binomial model, to test whether the probability of a population study with data being cited (anywhere within citation chain) by claimant sources was different for studies that found a negative association from those that did not find a negative association. 
...
summary(m)
```
The model summary table here gives a suite of information about the model fit (including AIC, BIC, log-likelihood), the amount of variance explained by each random effect (the claimant source `cited_by_id` and the prey species `scientificName`) and estimates for our coefficients. As you can see, there's no difference between citation probability for articles "in support" (estimate = -0.37, z = -0.859, p = 0.39) (also known as finding a negative association) as for articles that did not (the Intercept).

Now let's plot these probabilities:
```{r}
#| fig-width: 9
#| fig-height: 7
#| label: fig-citation-probabilities
#| fig-cap: "The probability of a population study with data being cited by a claimant source was not influenced by whether the population study found evidence of a negative association between cats and prey (according author's own interpretation)."
...
```
On average, claimants cited 17% of the available population studies that found a negative association between cats and prey and 23% of available populations studies that did not. All of these citations were made to make the claim that cats were the cause of prey decline.

# Summaries of evidence cited
Now let's look at the types of articles at the terminus of each citation chain. The y-axis on the following figures is the number of citation chains by terminus article type. This data was prepared in `scripts/1_data_mgmt/3 create probability of being cited dataset.R`.

## Quantitative articles {#sec-quantitative-evidence-freq}
We'll start by looking at all quantitative evidence uncovered, focused only on the citation chains from claimant sources. 
...
First, what percent of citation chain termini are not quantitative evidence?
...
out <- sub_terminus[, .(n = .N), by = evidence_simple]
out[, total := sum(n)]
out[, perc := n / total]
out
...
Now, let's plot the number of citation chains by terminus article type, focusing on quantitative evidence:
...
#| code-fold: true
#| label: fig-evidence
#| fig-cap: "Quantitative evidence cited found at the terminus of citation chains. X-axis indicates number of citation chains per evidence type."
...
The majority of evidence are predation records---that cats have been documented consuming or causing mortality to the prey species in question.

## Excluded articles {#sec-excluded-articles}

Now let's look at all the other terminus article types that did not qualify as quantitative evidence. We'll reload the same dataset we used [above](#sec-quantitative-evidence-freq) and we'll do some lumping and splitting of the categories used to classify the article types, simply to make the figure tractable.

```{r}
#| eval: true
#| echo: true
#| code-fold: true
#| fig-cap: "**Fig. 4.** Excluded evidence found at the terminus of citation chains. X-axis indicates number of citation chains per evidence type."
...
The majority of claims gave no citation, or ended with articles that made no claim about the hypothesis that cats caused endangerment of the prey species in question. Following this, expert opinion and inaccessible grey literature dominate the citation chain termini.
...
#| fig-cap: "The terminus of citation chains across the citation network. Each value (on x-axis) corresponds to a citation chain."
# 
