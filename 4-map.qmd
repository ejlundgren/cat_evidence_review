...
Otherwise, here are interactive map of all locations where quantitative evidence was found. The larger points indicate studies with data that could be meta-analyzed. 
...
sub_fill <- fill_pal[names(fill_pal) %in% droplevels(sub_dat$`Evidence type`)]
...
sub_fill <- fill_pal[names(fill_pal) %in% unique(droplevels(sub_dat$`Evidence type`))]
